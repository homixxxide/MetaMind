...
from PySide6.QtWidgets import (
    ...
    QDialog, QSplitter, QSizeGrip, QAbstractItemView, QMessageBox
)
...
class CVScanner:
    ...
    def __init__(self, api_keys):
        ...
        # Переиспользуем SIFT/FLANN (меньше нагрузки на CPU/RAM)
    ...
    def detect_heroes_with_sift(self, screenshot):
        ...
        aspect = w / max(h, 1)
        if aspect >= 2.2:
            top_portion = int(h * 0.19)
        elif aspect >= 1.9:
            top_portion = int(h * 0.22)
        else:
            top_portion = int(h * 0.25)
    ...
    def check_heroes_on_screen(self, force_check=False):
        ...
        # Кеширование результата на 1 секунду (снижаем лишние сканы)
        current_time = time.time()
        if not force_check and hasattr(self, '_heroes_check_cache') and hasattr(self, '_heroes_check_time'):
            if current_time - self._heroes_check_time < 1.0:
...
class MainWindow(QMainWindow):
    ...
    def _recommended_scan_interval_ms(self):
    ...
    def setup_cv_scanner(self):
        ...
        self.scan_interval_ms = self._recommended_scan_interval_ms()
        self.waiting_for_game = False

        print(f"[SETUP] ✅ CV Scanner готов, интервал {self.scan_interval_ms} ms")
    ...
    def _show_gsi_restart_dialog(self):
        msg = QMessageBox(self)
        msg.setIcon(QMessageBox.Information)
        msg.setWindowTitle("GSI готов")
        msg.setText("Конфиг GSI создан автоматически")
        msg.setInformativeText(
            "Чтобы автосканирование заработало:\n\n"
            "1) Полностью закрой Dota 2\n"
            "2) Запусти Dota 2 заново\n\n"
            "После перезапуска игра начнёт отправлять данные в MetaMind."
        )
        msg.setStyleSheet(f"""
            QMessageBox {{ background-color: #0b0f1a; }}
            QMessageBox QLabel {{ color: {COLOR_TEXT_WHITE}; font-size: 13px; }}
            QMessageBox QLabel#qt_msgbox_label {{ font-size: 15px; font-weight: bold; }}
            QPushButton {{
                background-color: {COLOR_ACCENT};
                color: white;
                border: none;
    ...
    def _show_support_dialog(self, reason_text: str):
        if getattr(self, '_support_dialog_shown', False):
            return
        self._support_dialog_shown = True
        msg = QMessageBox(self)
        msg.setIcon(QMessageBox.Warning)
        msg.setWindowTitle("Проблема подключения")
        msg.setText("Автосканирование ограничено")
        msg.setInformativeText(
            f"{reason_text}\n\n"
            "Проверь:\n"
            "• Брандмауэр/антивирус (доступ к localhost:3000)\n"
            "• Перезапуск Dota 2 после создания GSI\n"
            "• Запуск MetaMind от имени администратора\n\n"
            "Если не помогло — обратись в поддержку."
        )
        msg.exec()

    def _ensure_gsi_ready(self):
        gsi_found, gsi_path = self._find_gsi_config()
        if gsi_found:
            print(f"[GSI] ✅ Конфиг найден: {gsi_path}")
            return True

        print("[GSI] ⚠️ Конфиг не найден, пробуем создать автоматически")
        try:
            GSIServer()
        except Exception as e:
            print(f"[GSI] ❌ Ошибка автосоздания GSI: {e}")

        gsi_found, gsi_path = self._find_gsi_config()
        if gsi_found:
            print(f"[GSI] ✅ Конфиг создан: {gsi_path}")
            self._show_gsi_restart_dialog()
            self.on_scan_progress("⚠️ GSI создан — перезапусти Dota 2")
            return True

        self.on_scan_progress("❌ GSI конфиг не найден — автосканирование ограничено")
        self._show_support_dialog("MetaMind не может найти или создать GSI-конфиг.")
        return False

    def toggle_auto_scan(self, enabled):
        """Переключатель автосканирования (С ПОЛНЫМ СБРОСОМ)"""
        if enabled:
            self._support_dialog_shown = False
            self._ensure_gsi_ready()
            ...
            self.auto_scan_timer.start(self.scan_interval_ms)
            print(f"[TOGGLE] ✅ Быстрый SIFT loop ВКЛЮЧЕН ({self.scan_interval_ms} ms)")
    ...
    def get_team_from_gsi_cached(self):
        ...
        try:
            ...
            self._gsi_fail_streak = getattr(self, '_gsi_fail_streak', 0) + 1
            if self.cv_scanner.auto_scan_enabled and self._gsi_fail_streak >= 8:
                self._show_support_dialog("Нет ответа от локального GSI сервера (localhost:3000).")
